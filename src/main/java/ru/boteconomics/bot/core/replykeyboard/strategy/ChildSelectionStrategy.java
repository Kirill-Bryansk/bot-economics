...
/**
 * Стратегия выбора ребенка
 */
public class ChildSelectionStrategy implements KeyboardStrategy {

    @Override
    public ReplyKeyboardMarkup buildKeyboard(Object... context) {
        List<KeyboardRow> rows = new ArrayList<>();

        // Дети в столбик (по одному на строку для четкости)
        KeyboardRow row1 = new KeyboardRow();
        row1.add(ChildButton.ARTEMIY);
        rows.add(row1);

        KeyboardRow row2 = new KeyboardRow();
        row2.add(ChildButton.ARINA);
        rows.add(row2);

        KeyboardRow row3 = new KeyboardRow();
        row3.add(ChildButton.EKATERINA);
        rows.add(row3);
